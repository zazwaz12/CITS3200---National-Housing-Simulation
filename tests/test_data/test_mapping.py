...
class TestLoadGnafFilesByStates:
    ...
    def test_load_files_for_multiple_states(self, mock_scan_csv, mock_glob):
        ...
        # Expected data
        expected_geocode = pl.DataFrame({
            "ADDRESS_DETAIL_PID": ["1001", "1002", "1234", "4321"],  # Different PIDs for NSW and ACT
...
class TestFilterAndJoinGnafFrames:
    ...
    def address_detail_data(self):
        return pl.DataFrame({
            "ADDRESS_DETAIL_PID": ["1001", "1002", "1003"],
            "FLAT_TYPE_CODE": ["Flat", None, "Unit"],  # One null value to test null handling
    ...
    def test_filter_no_filters(self, default_geocode_data, address_detail_data):
        ...
        """
        result_lf = filter_and_join_gnaf_frames(default_geocode_data, address_detail_data)
        expected_df = pl.DataFrame({
            "ADDRESS_DETAIL_PID": ["1001", "1002", "1003"],
            "LATITUDE": [34.5, 35.0, 36.0],
            "LONGITUDE": [150.3, 149.1, 148.5],
            "FLAT_TYPE_CODE": ["flat", "unknown", "unit"],  # "Flat" converted to lowercase, None replaced with "unknown"
            "POSTCODE": [2000, 2600, 3000]
        })
    ...
    def test_filter_by_building_type(self, default_geocode_data, address_detail_data):
        ...
        """
        result_lf = filter_and_join_gnaf_frames(default_geocode_data, address_detail_data, building_types=["flat"])
        expected_df = pl.DataFrame({
    ...
    def test_filter_by_postcode(self, default_geocode_data, address_detail_data):
        ...
        """
        result_lf = filter_and_join_gnaf_frames(default_geocode_data, address_detail_data, postcodes=[2600])
        expected_df = pl.DataFrame({
    ...
    def test_filter_by_building_type_and_postcode(self, default_geocode_data, address_detail_data):
        """
        Test the function with both building type and postcode filters applied.
        """
        result_lf = filter_and_join_gnaf_frames(default_geocode_data, address_detail_data, building_types=["unit"], postcodes=[3000])
        expected_df = pl.DataFrame({
    ...
    def test_no_matching_postcode(self, default_geocode_data, address_detail_data):
        ...
        """
        result_lf = filter_and_join_gnaf_frames(default_geocode_data, address_detail_data, postcodes=[9999])
        expected_df = pl.DataFrame({
            "ADDRESS_DETAIL_PID": [],
            "LATITUDE": [],
            "LONGITUDE": [],
